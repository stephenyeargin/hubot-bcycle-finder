...
const script = require('../../src/bcycle-finder');
...
async function createTestBot(settings = {}) {
  ...
  if ('BCYCLE_CITY' in settings) {
    if (settings.BCYCLE_CITY === null) {
      delete process.env.BCYCLE_CITY;
    } else {
      process.env.BCYCLE_CITY = settings.BCYCLE_CITY;
    }
  } else {
    process.env.BCYCLE_CITY = 'nashville';
  }
  if ('BCYCLE_DEFAULT_STATIONS' in settings) {
    if (settings.BCYCLE_DEFAULT_STATIONS === null) {
      delete process.env.BCYCLE_DEFAULT_STATIONS;
    } else {
      process.env.BCYCLE_DEFAULT_STATIONS = settings.BCYCLE_DEFAULT_STATIONS;
    }
  } else {
    process.env.BCYCLE_DEFAULT_STATIONS = '123';
  }
  nock.cleanAll();
